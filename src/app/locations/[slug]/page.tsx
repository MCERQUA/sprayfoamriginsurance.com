...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description: `${loc.blurb} Inland marine at replacement cost, equipment breakdown, rig truck auto, and CPL for ${loc.name} spray foam rig operators. 15-minute quotes.`,
  ...
  return (
    <>
      ...
      <main>
            ...
            <div className="grid lg:grid-cols-12 gap-10 items-center">
              ...
              <FadeIn direction="left" className="lg:col-span-5">
                <div className="rounded-t-[10rem] rounded-b-3xl overflow-hidden border-4 border-white shadow-warm-lg">
                  <img src="/images/coverage.jpg" alt={`Spray foam rig operator in ${loc.name}`} className="w-full h-[300px] md:h-[360px] object-cover" loading="lazy" />
                </div>
              </FadeIn>
            </div>
          </div>
        ...
        <section className="bg-cream py-16 md:py-20">
          <div className="container-tight">
            <FadeIn className="max-w-2xl mb-10">
              <span className="eyebrow"><span className="h-px w-8 bg-clay" />Rig coverage for {loc.name} spray foam contractors</span>
              <h2 className="mt-3 h-section">The full rig program for {loc.name} spray foam operators.</h2>
              <p className="mt-4 lead">From owner-operators to multi-rig fleets, we coordinate inland marine, breakdown, auto, and GL for {loc.name} spray foam rigs.</p>
            </FadeIn>
        ...
        <FAQ items={locFaqs} eyebrow={`${loc.name} — FAQ`} title={<>Spray foam rig insurance questions for <span className="text-clay">{loc.name}</span></>} background="sand" />

        <CTABand title={`Insuring spray foam rigs in ${loc.name} since 2005`} description={`Local knowledge, A-rated markets, and 15-minute quotes. Call ${SITE.phone} or request a quote online.`} />
      </main>
      <Footer />
    </>
  );
